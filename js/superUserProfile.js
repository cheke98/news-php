...
var id=localStorage.getItem("idUserSuperToShow");
$.ajax({
    ...
    success: function (data) {
      ...
      if(info.length>0){
        for(let i=0;i<info.length;i++){
          ...
          dFlexJust.append(g);
          var d = '<span class="stories__emoji" onclick="delArticle(\''+info[i]['id']+'\')">🚽</span>';
          dFlexJust.append(d);
...
function countComments(id,user_type){
  $.ajax({
    url: "../php/mysqlSelectAllCommentsSuper.php",
    type: "POST",
    data: { id:id },
    dataType: "jsonp",
    jsonp: "jsoncallback",
...
function getReportersStats(){
  var id=localStorage.getItem("idUserSuperToShow");
  $.ajax({
    url: "../php/mysqlSelectAllReporters.php",
    type: "POST",
    data: { id:id },
    dataType: "jsonp",
    jsonp: "jsoncallback",
...
function summarize(){
 ...
 setTimeout(function(){ 
    ...
    myRepoLikes = (myRepoLikes * 4) * .6;
    myRepoCom = (myRepoCom * 2) * .6;
    ...
    success: function (data) {
      ...
      if(reporters.length>0){
        ...
        for(let i=0;i<reporters.length;i++){
         ...
         setTimeout(function(){
          var count = parseInt(localStorage.getItem("totalReportersArticles"),10);
         $("#profileReportersSubtitle").text("Las de tus reporteros("+count+")");
    ...
    success: function (data) {
      ...
      var container = $("#reporters-stories-container");
        ...
        var li = '<li id="reporter-'+id+'-storie-'+i+'"></li>';
              ...
              var dflex = '<div id="dflex-'+id+'-'+i+'" class="d-flex justify-content-between align-items-center"></div>'; 
              li.append(dflex);
              dflex=$("#dflex-"+id+"-"+i);
              var divdesc = '<div id="story-desc-'+id+'-'+i+'" class="stories__description" onclick="showArticle(\''+articles[i]['title']+'\')"></div>';
              dflex.append(divdesc);
              divdesc = $("#story-desc-"+id+"-"+i);
              var title = '<span class="stories__title">'+articles[i]['title']+'</span>';
              divdesc.append(title);
              var br = '<br>';
              divdesc.append(br);
              var subtitle = '<span class="stories__subtitle">'+articles[i]['subtitle']+'</span>';
              divdesc.append(subtitle);
              divdesc.append(br);
              var rep = '<span class="stories__subtitle">Por: '+articles[i]['name']+'</span>';
              divdesc.append(rep);
              var dflexcol = '<div id="d-flex-'+id+'-'+i+'" class="d-flex flex-column"></div>';
              dflex.append(dflexcol);
              dflexcol = $("#d-flex-"+id+"-"+i);
              dflexjust = '<div id="d-flex-just-'+id+'-'+i+'" class="d-flex justify-content-around"></div>';
              dflexcol.append(dflexjust);
              dflexjust = $("#d-flex-just-"+id+"-"+i);
               var g = '<span class="stories__emoji" onclick="generateStats('+articles[i]['likes']+','+articles[i]['id']+')">📊</span>';
              dflexjust.append(g);
              var d = '<span class="stories__emoji" onclick="delArticle(\''+articles[i]['id']+'\')">🚽</span>';
              dflexjust.append(d);
              var create = '<span class="stories__date">'+articles[i]['creation_date']+'</span>';
              dflexcol.append(create);
        }
      }   
  },
    ...
    success: function (data) {
      ...
      if(requests.length>0){
        ...
        for(let i=0;i<requests.length;i++){
          ...
          ul.append(li);
          li = $("#request-"+i);
          var dflexreq = '<div id="d-flex-req-'+i+'" class="d-flex flex-row justify-content-between"></div>';
          li.append(dflexreq);
          dflexreq = $("#d-flex-req-"+i);
          var aprofile = '<a id="a-to-profile-'+i+'" class="profile__admins-requests-reporter" href="profileFromAdmin.html" onclick="goToProfile(\''+requests[i]['id_user_reporter']+'\')"></a>';
          dflexreq.append(aprofile);
          aprofile = $("#a-to-profile-"+i);
          var name = '<span onclick="goToProfile(\''+requests[i]['id_user_reporter']+'\')">'+requests[i]['name']+'</span>';
          aprofile.append(name);
          var dflexbtns='<div id="d-flex-btns-'+i+'" class="d-flex flex-row"></div>';
          dflexreq.append(dflexbtns);
          dflexbtns = $("#d-flex-btns-"+i);
          var acepta = '<a id="refresh-acept-'+i+'" href="adminProfile.html" onclick="acceptRequest(\''+requests[i]['id']+'\',\''+requests[i]['id_user_reporter']+'\',\''+id+'\')"></a>';
          dflexbtns.append(acepta);
          acepta = $("#refresh-acept-"+i);
          var spanAcept = '<span class="profile__admins-requests-acept mr-3">Aceptar</span';
          acepta.append(spanAcept);
          var avoid = '<a id="refresh-avoid-'+i+'" href="adminProfile.html" onclick="refuseRequest(\''+requests[i]['id']+'\')"></a>';
          dflexbtns.append(avoid);
          avoid = $("#refresh-avoid-"+i);
          var spanAvoid = '<span class="profile__admins-requests-reject">Rechazar</span>';
